...
/*!
 * diagram-parser - Parser for diagrams.io
 * v0.1.0
 * 
 * copyright First+Third 2014
 * MIT License
 ...
 * MIT License
*/
(function(e){var t=function(){var n=Array.prototype.slice.call(arguments),r=!1,i=n.shift(),s="";if(typeof i=="string"||typeof i=="boolean")s=i===!0?"deep":i,i=n.shift(),s=="defaults"&&(i=t({},i),s="strict");for(var o=0,u=n.length;o<u;o++){var a=n[o];for(var f in a)if(s=="deep"&&typeof a[f]=="object"&&typeof i[f]!="undefined")t(s,i[f],a[f]);else if(s!="strict"||s=="strict"&&typeof i[f]!="undefined")i[f]=a[f]}return i};typeof module!="undefined"?module.exports=t:(typeof define=="function"&&define.amd&&define(function(){return t}),e.aug=t)})(this);
(function () {
  "use strict";
  ...
  function getTabChar(array) {
    var possibilities = [' ', '\t'],
        tabChar = null;

    for (var i = 0, len = array.length; i < len; i++) {
      var line = array[i],
          index = possibilities.indexOf(line[0]);

      if (index > -1){
        tabChar = possibilities[index];
        var j = 1;

        while (line[j] === tabChar){
          j++;
        }

        tabChar = repeat(tabChar, j);
  ...
  function parseLine(line){
    ...
    var trimmedLine = line.trim(),
        indexOfStart = trimmedLine.indexOf('['),
        indexOfEnd = trimmedLine.indexOf(']'),
        extraLine, extraAttrs;

    if (indexOfStart > -1){
      if (indexOfEnd > -1){
        extraLine = trimmedLine.substring(indexOfStart + 1, indexOfEnd);

        if(indexOfStart === 0){
          result.name = trimmedLine.substring(indexOfEnd +1, trimmedLine.length).trim();
        }
        else {
          result.name = trimmedLine.substring(0, indexOfStart).trim();
        }

        extraAttrs = extraLine.split(',');
        ...
        throw 'Found [ but not matching ending ] in "' +  line + '"';
      }
    }
  ...
  window.DiagramParser = function (content) {
    var lines = content.replace(/\r\n/g, '\n').split('\n');
    var TAB_CHAR = getTabChar(lines), TAB_CHAR_REGEX,
        currentTree = {},
        result;

    TAB_CHAR_REGEX = new RegExp(TAB_CHAR, 'gi');
    ...
    lines.forEach(function (line) {
      var matches = line.match(TAB_CHAR_REGEX),
          count =  matches ? matches.length : 0,
          parent = count - 1,
          existingNode,
          parsed = parseLine(line.replace(TAB_CHAR_REGEX,''));

      if (!parsed.name.length){
        return;
      }

      existingNode = result.allNodes.map(function(o) { return o.name; }).indexOf(parsed.name);

      if (existingNode < 0){
        result.allNodes.push(aug({}, parsed));
      }
      else {
        result.allNodes[existingNode] = aug({}, result.allNodes[existingNode], parsed);
      }
      ...
      if (count){
        parsed.from = currentTree[parent].name;
        parsed.to = parsed.name;

        delete parsed.name;

        result.connections.push(aug({}, parsed));
      }
    });
